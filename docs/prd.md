...
- **NFR1**: 分支切换增加的延迟不超过 500ms（stash 操作）
- **NFR2**: Stash 检索和匹配操作在 100ms 内完成
- **NFR3**: 命令启动时间不超过 200ms（冷启动）

### 可靠性要求

- **NFR4**: 自动 stash 操作必须原子性执行，失败时回滚到操作前状态
- **NFR5**: 任何情况下不得丢失用户的未提交更改
- **NFR6**: 网络操作（pull/push）超时时提供清晰的错误信息和恢复建议

### 兼容性要求

- **NFR7**: 支持 Git 2.20+ 版本
- **NFR8**: 支持 Node.js ^20.19.0 || >=22.12.0
- **NFR9**: 兼容 macOS、Linux、Windows（通过 WSL）
- **NFR10**: 与现有 ng 命令完全向后兼容

### 可维护性要求

- **NFR11**: 新功能代码遵循现有项目的 TypeScript 规范
- **NFR12**: 新命令与现有命令风格一致（Commander.js + @clack/prompts）
- **NFR13**: 关键功能需有单元测试覆盖
...
| **功能需求数量** | 30 个 |
| **非功能需求数量** | 13 个 |
| **MVP 范围** | 分支级 Stash、Pull 安全模式、Push 安全模式、Rebase 优化 |
| **核心价值** | 让 Git 操作像呼吸一样自然 |
