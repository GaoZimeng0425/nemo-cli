...
import { log } from '../log' // Relative to this file's new location
import { getPackageDependencies } from '../packageJson' // Relative to this file's new location
...
describe('getPackageDependencies', () => {
  ...
  it('should return both dependencies and devDependencies', async () => {
    ...
    expect(mockLog.info).toHaveBeenCalledWith(`Reading package.json from: ${path.join(mockPackageDir, 'package.json')}`)
  })
  ...
  it('should return only dependencies if devDependencies are missing', async () => {
    ...
    const dependencies = await getPackageDependencies(mockPackageDir)
    expect(dependencies).toEqual([{ name: 'lodash', version: '^4.17.21', type: 'dependencies' }])
  })
  ...
  it('should return only devDependencies if dependencies are missing', async () => {
    ...
    const dependencies = await getPackageDependencies(mockPackageDir)
    expect(dependencies).toEqual([{ name: 'eslint', version: '^8.0.0', type: 'devDependencies' }])
  })
  ...
  it('should return an empty array and log error if package.json is not found', async () => {
    ...
    const dependencies = await getPackageDependencies(mockPackageDir)
    expect(dependencies).toEqual([])
    expect(mockLog.error).toHaveBeenCalledWith(`package.json not found at ${path.join(mockPackageDir, 'package.json')}`)
  })
  ...
  it('should return an empty array and log error if package.json is malformed', async () => {
    ...
    expect(mockLog.error).toHaveBeenCalledWith(
      expect.stringContaining(`Failed to read or parse package.json at ${path.join(mockPackageDir, 'package.json')}`)
    )
  })
