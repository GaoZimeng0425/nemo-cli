...
## 6. branch list - 交互式分支查看器 (NEW 2026-02-09)
...
| 选项 | 说明 |
|------|------|
| `-l, --local` | 仅列出本地分支（简单输出模式） |
| `-r, --remote` | 仅列出远程分支（简单输出模式） |
| `-a, --all` | 交互式查看器（默认） |
| `-n, --number <count>` | 限制显示的分支数量 |

### 功能

#### 交互式查看器模式（默认）

当使用 `-a/--all` 或不指定 `-l/-r` 时，启动交互式双面板查看器：
